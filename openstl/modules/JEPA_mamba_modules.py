...
# This is what we want to use to encode each image in the video
# We will use the output of this model as the input to an MLP predictor
# We can squeeze the frames of the video into the batch dimension
# and then pass it through this model
# The output of this model will be the input to the MLP predictor after we unsqueeze the batch dimension
...
class VMAMBA2_Video_Encoder(nn.Module):
    def __init__(self, backbone, num_frames, num_classes, hidden_dim=512, num_layers=3, **kwargs):
        super(VMAMBA2_Video_Encoder, self).__init__()
